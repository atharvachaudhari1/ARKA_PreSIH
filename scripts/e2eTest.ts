  ...
  try {
    ...
    const team = await prisma.team.create({
      data: {
        ...
        slots: {
          create: [
            { role_title: 'Frontend', is_filled: true, filled_by_user_id: leader.id },
            { role_title: 'Backend', is_filled: false }
          ]
        }
      },
      include: { slots: true, memberships: true }
    });

    if (team.memberships.length !== 1 || team.slots.length !== 2) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (emptySlot) {
        await tx.teamSlot.update({
          ...
          data: { is_filled: true, filled_by_user_id: applicant.id }
        });
      }
